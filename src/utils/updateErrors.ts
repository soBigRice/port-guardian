...
/**
 * 将 Tauri updater 或网络层返回的错误转换成用户能判断下一步的文案。
...
export function formatUpdateError(
  ...
  const message = raw || (t ? t("updateErrors.unknown") : "未知错误");
  const lower = message.toLowerCase();

  if (message.includes("404") || lower.includes("not found")) {
    return t ? t("updateErrors.notFound") : "更新源未找到 latest.json，请确认 GitHub Release 已公开并包含更新元数据。";
  }

  if (lower.includes("signature") || lower.includes("pubkey") || lower.includes("public key")) {
    return t ? t("updateErrors.signature") : "更新签名校验失败，请确认发布私钥和应用内公钥匹配。";
  }

  if (lower.includes("network") || lower.includes("timeout") || lower.includes("failed to fetch")) {
    return t ? t("updateErrors.network") : "更新源连接失败，请检查网络或稍后重试。";
  }
